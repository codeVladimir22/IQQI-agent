 ...
 * origin=git@git.iqqi.ai:iqqi/IQQI-agent.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@git.iqqi.ai:iqqi/IQQI-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@git.iqqi.ai:iqqi/IQQI-agent'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@git.iqqi.ai/iqqi/IQQI-agent.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://git.iqqi.ai/IQQI-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@git.iqqi.ai:iqqi/iqqi-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://git.iqqi.ai/IQQI-agent/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@git.iqqi.ai:iqqi/IQQI-agent.git'), true)
  assert.equal(isSshRemote('ssh://git@git.iqqi.ai/iqqi/IQQI-agent.git'), true)
  assert.equal(isSshRemote('https://git.iqqi.ai/IQQI-agent.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@git.iqqi.ai:iqqi/IQQI-agent.git'), true)
  assert.equal(isOfficialSshRemote('git@git.iqqi.ai:iqqi/IQQI-agent'), true)
  assert.equal(isOfficialSshRemote('ssh://git@git.iqqi.ai/iqqi/IQQI-agent.git'), true)
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@git.iqqi.ai:someuser/IQQI-agent.git'), false)
  // Same repo name on a different host is not the official repo.
  assert.equal(isOfficialSshRemote('git@gitlab.com:iqqi/IQQI-agent.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://git.iqqi.ai/IQQI-agent.git'), false)
