...
# ============================================================================
# PATH setup — symlink iqqi into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up iqqi command..."

IQQI_BIN="$SCRIPT_DIR/venv/bin/iqqi"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$IQQI_BIN" "$COMMAND_LINK_DIR/iqqi"
ln -sf "$IQQI_BIN" "$COMMAND_LINK_DIR/hermes"
echo -e "${GREEN}✓${NC} Symlinked iqqi → $COMMAND_LINK_DISPLAY_DIR/iqqi"
echo -e "${GREEN}✓${NC} Kept hermes compatibility alias → $COMMAND_LINK_DISPLAY_DIR/hermes"
...
echo "Other commands:"
echo "  iqqi status          # Check configuration"
if is_termux; then
    echo "  iqqi gateway         # Run gateway in foreground"
else
    echo "  iqqi gateway install # Install gateway service (messaging + cron)"
fi
echo "  iqqi cron list       # View scheduled jobs"
echo "  iqqi doctor          # Diagnose issues"
echo ""
